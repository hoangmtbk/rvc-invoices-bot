...
Add: `DB_PATH = os.path.join(DATA_DIR, "invoices.db")`, `WEB_SECRET`, `WEB_PORT`, `MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_BUCKET`, `MINIO_PUBLIC_URL`
...
Minimal Flask app — read-only. Protected by a secret token query parameter.

### Authentication

Every request to `/` and `/export` must include `?secret=<WEB_SECRET>`. Flask checks this on every request via a `before_request` hook:
- Missing or wrong token → `403 Forbidden`
- Correct token → request proceeds

The secret is preserved automatically in all pagination and export links generated by the template (appended as `&secret=...`). Users can bookmark the URL with the secret included.

`WEB_SECRET` is loaded from `.env`. If not set, the server refuses to start.

### Routes

| Route | Description |
|---|---|
| `GET /?secret=<token>` | Invoice table with filters and column toggles |
| `GET /export?secret=<token>` | Download CSV |
